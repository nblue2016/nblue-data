...
class MongoDbConnections extends EventEmitter {
  ...
  create (name, connectionString, options) {
    ...
    const conn = ctx._createConnection(name, connectionString, options, ctx)
  ...
  _createConnection (name, connectionString, options, caller) {
    if (!connectionString) {
