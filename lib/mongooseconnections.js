...
const MongooseDbAdapter = require('./mongooseadapter')
...
class MongooseConnections extends EventEmitter {
  ...
  define (schemas) {
    const cache = this.Schemas

    cache.createSchemas(schemas, schemaType)
  }
  ...
  createByConfig (name, config) {
    ...
    const {
      connectionString: c1,
      options: o1
    } = MongooseConnections.parseUrl(dbUrl)
  ...
  createAdapter (name) {
    const ctx = this
    const adapter = new MongooseDbAdapter(name, ctx)
