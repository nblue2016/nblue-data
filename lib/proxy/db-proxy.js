...
const DbConnection = require('./db-connection2')
...
class DbProxy
{
  ...
  createConnection (name, connectionString, options) {
    const ctx = this

    const { cs, opts } = ctx.parseUrl(connectionString, options)

    const conn = new DbConnection(ctx)
