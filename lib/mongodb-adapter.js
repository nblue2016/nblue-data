...
class MongoDbAdapter extends DbAdapter
{
  ...
  getCollecton (caller) {
    ...
    const collection = mongoDbConn.collection(name)
  ...
  _create (body, options) {
    ...
    return (resolver) => Promise.
      ...
      then(() => {
        ...
        return collection[method](body, options, resolver)
      }).
      catch((err) => {
  ...
  _retrieve (filter, options) {
    ...
    if (method === 'findOne') {
      ...
      return (resolver) => {
        collection[method](filter, oneOptions, resolver)
      }
    } else if (method === 'count') {
      return (resolver) => collection[method](filter, options, resolver)
    }
  ...
  _update (filter, modifier, options) {
    ...
    return (resolver) =>
        collection[method](filter, { $set: modifier }, options, resolver)
  }
