...
class Schemas
{
  ...
  static validateEntity (entity, model, options) {
      // parse options
    ...
    entityKeys.forEach((key) => {
      ...
      if (modelItems.isArray) {
        ...
        val.forEach((item) => {
          if (type === 'object') {
            Schemas.validateEntity(item, model[key].model, newOptions)
          } else {
        ...
        newOptions.base = key
        Schemas.validateEntity(entity[key], model[key].model, newOptions)
      } else {
  ...
  parseSchemas (data) {
    ...
    Object.
      ...
      map((name) => {
        ...
        result.model = ctx.convertModel(item.model)
  ...
  validateSchema (name, entity) {
    ...
    const model = Schemas.getModel(schema)
