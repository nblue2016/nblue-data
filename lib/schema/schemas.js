...
class Schemas
{
  ...
  static parse (file, callback) {
    ...
    const pending =
      Array.isArray(file)
      ? Promise.all(
        file.map((item) => sc.readFile(item))
      )
  ...
  static validateEntityBySchema (entity, schema) {
    ...
    Schemas.validateEntityByModel(entity, Schemas.getModel(schema), opts)
  }

  static validateEntityByModel (entity, model, options) {
      // parse options
    ...
    entityKeys.forEach((key) => {
      ...
      if (modelItems.isArray) {
        ...
        val.forEach((item) => {
          if (type === 'object') {
            Schemas.validateEntityByModel(item, model[key].model, newOptions)
          } else {
            Schemas.validateType(key, item, type, newOptions)
          }
        })
      } else if (type === 'object') {
        newOptions.base = key
        Schemas.validateEntityByModel(entity[key], model[key].model, newOptions)
      } else {
        Schemas.validateType(key, val, type, newOptions)
      }
    })
  }
  ...
  readFile (file, callback) {
    ...
    const workflow = {
      ...
      _r3: (ctx$, data) =>
        ctx.parseFile(data, ctx.getExtName(file))
    }
