...
class OrmDbAdapter extends DbAdapter
{
  ...
  _create (body, options) {
    ...
    return aq.back((cb) => createFunc(body, cb))
  }
  ...
  _retrieve (filter, options) {
    ...
    if (methodName === 'find') {
      return aq.back((cb) => retrieveFunc(filter, opts, cb))
    }

    return aq.back((cb) => retrieveFunc(filter, cb))
  }
      ...
      const rt = yield aq.parallel(
        data.map(
          (item) => {
            ...
            return aq.back((cb) => item.save(cb)).
              then(() => 1).
              catch(() => 0)
          }
        )
