...
class MongooseDbAdapter extends DbAdapter
{
  ...
  _retrieve (filter, options) {
    ...
    return aq.back((cb) => pending.exec(cb))
  }
  ...
  _update (filter, modifier, options) {
    ...
    return aq.back((cb) => model.update(filter, modifier, opts, cb))
  }
  ...
  _delete (filter, options) {
    ...
    return aq.back((cb) => model.remove(filter, cb))
  }
