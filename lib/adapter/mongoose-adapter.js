...
class MongooseDbAdapter extends DbAdapter
{
    ...
    return Promise.
        ...
        then(() => {
          ...
          if (options.autoIncrement) {
            return that._increment(item, options)
          }

          return Promise.resolve(0)
        }).
        then(() =>
          that._createPromise((cb) => item.save(cb))
        ).
        catch((err) => {
  ...
  _retrieve (filter, options) {
    ...
    return that._createPromise((cb) => pending.exec(cb))
  }
  ...
  _update (filter, modifier, options) {
    ...
    return that._createPromise(
      (cb) => that.Model.update(filter, modifier, opts, cb)
    )
  }
  ...
  _delete (filter, options) {
    ...
    return that._createPromise(
      (cb) => that.Model.remove(filter, cb)
    )
  }
