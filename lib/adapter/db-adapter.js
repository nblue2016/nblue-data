...
class DbAdapter extends EventEmitter {
  ...
  setDefaultValues (data, caller) {
    const ctx = caller ? caller : this

    if (Array.isArray(data)) {
      data.
        forEach(
          (item) => ctx.setDefaultValues(item)
        )
      ...
      Object.
        ...
        forEach((key) => {
          ...
          if (typeof modelDefined === 'object') {
            if (modelDefined.default) {
              if (typeof modelDefined.default === 'function') {
                idata[key] = Reflect.apply(modelDefined.default, idata, [])
              } else {
                idata[key] = modelDefined.default
              }
            } else if (!idata[key]) {
              if (Array.isArray(imodel[key])) {
                idata[key] = []
              } else if (!modelDefined.type) {
                idata[key] = {}
