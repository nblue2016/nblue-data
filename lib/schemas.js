  ...
  convertModel (model) {
    ...
    Object.
      ...
      forEach((key) => {
        ...
        items.isArray = false
        items.type = typeof modelItems === 'string' ? modelItems : 'string'
        items.require = false
        ...
        if (Array.isArray(modelItems)) {
          ...
          if (!modelItems.type) {
            ...
            newModel[key].type = 'object'

            return
          }


          if (modelItems.type) items.type = modelItems.type
    ...
    modelKeys.
      ...
      forEach((key) => {
        if (!(keys.includes(key) || entity[key])) {
          throw new ValidatorError(`The key: ${key} is required.`)
        }
      })

    keys.forEach((key) => {
      if (methodKeys.includes(key)) return

      // check keys
      if (!modelKeys.includes(key)) {
        throw new ValidatorError(`Can't find ${key} in model`)
      }
      ...
      if (modelItems.isArray) {
        if (!(Array.isArray(val) && val.length === 0)) {
          throw new ValidatorError(`Invaild array type for ${key}`)
        }
  ...
  verifyType (key, val, type) {
    ...
    case 'string':
      if (typeof val !== 'string') {
        throw new ValidatorError(`invalid string for key ${key}`)
      }
      break
    case 'number':
      try {
        ...
        throw new ValidatorError(`invalid number value for ${key}`)
      }
      break
    case 'date':
      try {
        ...
        throw new ValidatorError(`invalid date value for ${key}`)
      }
      break
    case 'object':
      break
    default:
      throw new ValidatorError(`Doesn't support type: ${type} of key: ${key}`)
    }
  }
