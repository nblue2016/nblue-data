...
class DbAdapter extends EventEmitter {
  ...
  create (body, callback) {
    ...
    if (autoIncrement === null) {
      ...
      promiseFunc = ctx.createPromise(callBackFunc)
    ...
    return ctx.
      resolvePromise(
        promiseFunc(newBody, autoIncrement, promiseFunc, callBackFunc),
        resultFunc,
        callback
      )
  }
  ...
  count (filter, callback) {
    ...
    // return ctx.retrieve(filter, { countOnly: true }, callback)
  }
  ...
  retrieve (filter, options, callback) {
    ...
    const callBackFunc = (model, resolver) => {
      // check method
    ...
    const promiseFunc = ctx.createPromise(callBackFunc)
